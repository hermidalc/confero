package Confero::DataFile::IdList;
...
sub _load_data_from_file {
    ...
    open(my $id_list_fh, '<', $self->file_path) or confess('Could not open data file ' . $self->file_path . ": $!");
    while (<$id_list_fh>) {
        s/^\s+//;
        s/\s+$//;
        ...
        if (m/^#%/) {
            ...
            $field_name = $field_name eq 'geneset_name' ? 'gene_set_name'   :
                          $field_name eq 'geneset_desc' ? 'gene_set_desc'   : 
        ...
        if (!$read_first_non_header_line) {
            ...
            # if organism name, dataset/gene set name, or dataset/gene set desc not passed to object constructor then use header metadata
            $self->{organism_name} = $self->metadata->{organism} if !defined $self->organism_name and 
                                                                     defined $self->metadata->{organism};
            $self->{dataset_name} = $self->metadata->{dataset_name} if !defined $self->gene_set_name and 
                                                                        defined $self->metadata->{dataset_name};
            $self->{dataset_name} = $self->metadata->{gene_set_name} if !defined $self->gene_set_name and 
                                                                         defined $self->metadata->{gene_set_name};
            $self->{dataset_desc} = $self->metadata->{dataset_desc} if !defined $self->gene_set_desc and 
                                                                        defined $self->metadata->{dataset_desc};
            $self->{dataset_desc} = $self->metadata->{gene_set_desc} if !defined $self->gene_set_desc and 
                                                                         defined $self->metadata->{gene_set_desc};
            ...
            # extract from original file basename possible gene_set_name, contrast_name and gene_set_type
            (
            ...
            # if dataset_name/gene_set_name starts with $CTK_DISPLAY_ID_PREFIX remove it also convert any 
            # $CTK_DISPLAY_ID_SPACER characters in gene_set_name, contrast_name and gene_set_type into a space
            if (defined $self->gene_set_name) {
            ...
            if (defined $self->metadata->{gene_set_name}) {
                $self->metadata->{gene_set_name} =~ s/^${CTK_DISPLAY_ID_PREFIX}${CTK_DISPLAY_ID_SPACER}//go;
                $self->metadata->{gene_set_name} =~ s/$CTK_DISPLAY_ID_SPACER/ /go;
            }
            $self->{contrast_name} =~ s/$CTK_DISPLAY_ID_SPACER/ /go if defined $self->contrast_name;
            $self->{gene_set_type} =~ s/$CTK_DISPLAY_ID_SPACER/ /go if defined $self->gene_set_type;
...
sub _process_data {
    ...
    if (!$self->has_gene_ids) {
        ...
        for my $data_row (@{$self->source_data}) {
            ...
            my @data_fields = split /\t/, $data_row->{data_str} if defined $data_row->{data_str};
...
sub write_processed_file {
    ...
    # add gene_set_name and gene_set_desc if not in metadata header
    unshift @{$self->_raw_metadata}, qq/#%gene_set_name="$self->{dataset_name}"/
        if (!defined $self->metadata->{dataset_name} and !defined $self->metadata->{gene_set_name}) and defined $self->{dataset_name};
    unshift @{$self->_raw_metadata}, qq/#%gene_set_desc="$self->{dataset_desc}"/
        if (!defined $self->metadata->{dataset_desc} and !defined $self->metadata->{gene_set_desc}) and defined $self->{dataset_desc};
